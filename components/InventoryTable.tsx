...
import { Search, CheckCircle2, Factory, ArrowRight, X, Scale, AlertTriangle, Truck, ShoppingCart, RefreshCw, Filter, Calendar, Trash2, Edit2 } from 'lucide-react';
...
const InventoryTable: React.FC<Props> = ({ batches, partners, materials, onUpdateStatus, onDeleteBatch, onEditBatch }) => {
  ...
  const handleModalSubmit = (e: React.FormEvent) => {
    ...
    if (modalType === 'sell' && weightToUse > activeBatch.weightKg) {
    ...
    onUpdateStatus(activeBatch.id, modalType === 'finalize' ? 'finished' : modalType === 'extrude_send' ? 'extruding' : modalType === 'extrude_return' ? 'extruded' : 'sold', config);
    resetModal();
  };
  ...
  const openModal = (type: typeof modalType, batch: Batch) => {
    ...
    setFormPartnerId(''); // Limpa para forçar seleção de cliente/prestador
    setFormPrice('');
  ...
  const isPartialSale = activeBatch && modalType === 'sell' && parseFloat(formWeight) < activeBatch.weightKg;
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-200 border border-slate-200">
            <div className="bg-[#000814] p-6 text-white flex justify-between items-center">
              <h3 className="text-xl font-bold flex items-center gap-2">
                ...
                {modalType === 'sell' ? 'Venda de Material' : 'Editar Lote'}
            ...
            <form onSubmit={handleModalSubmit} className="p-8 space-y-6">
              ...
              {modalType === 'sell' && (
                <div className="space-y-4">
                    <div className="space-y-1">
                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Peso a Vender (Kg)</label>
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className={`w-full p-4 pl-10 border rounded-2xl text-2xl font-black outline-none transition ${isPartialSale ? 'bg-amber-50 border-amber-300 text-amber-700' : 'bg-slate-50 border-slate-200 text-brand-700'}`} 
                            />
                        </div>
                        <div className="flex justify-between items-center mt-2 px-1">
                            ...
                            {isPartialSale ? (
                                <div className="flex items-center gap-1 text-amber-600 text-[10px] font-black uppercase">
                                    <AlertTriangle className="w-3 h-3" /> Venda Parcial
                                </div>
                            ) : (
                                <div className="flex items-center gap-1 text-brand-600 text-[10px] font-black uppercase">
                                    <CheckCircle2 className="w-3 h-3" /> Venda Total
                                </div>
                            )}
                        </div>
                        {isPartialSale && (
              ...
              <button 
                type="submit" 
                className="w-full py-4 bg-brand-600 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-brand-700 shadow-xl shadow-brand-100 transition active:scale-95"
              >
                Confirmar e Salvar
              </button>
            </form>
