          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 text-[10px] uppercase font-bold text-slate-500 tracking-widest border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-brand-600">Vencimento</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map(entry => (
                <tr key={entry.id} className="hover:bg-slate-50/50 transition group">
                  <td className="px-6 py-4 text-[10px] text-slate-500">{new Date(entry.date).toLocaleDateString()}</td>
                  <td className="px-6 py-4 text-xs font-bold text-brand-700">{new Date(entry.dueDate).toLocaleDateString()}</td>
                  <td className="px-6 py-4">
                    <div className="text-sm font-bold text-slate-800">
                        {partners.find(p => p.id === entry.partnerId)?.name || 'N/A'}
                    </div>
                  </td>
                  <td className={`px-6 py-4 text-right font-black ${entry.type === 'payable' ? 'text-slate-900' : 'text-brand-600'}`}>
                    ...
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {entry.status === 'pending' && (
                        <button 
                          ...
                          className="text-[10px] font-black text-brand-600 hover:underline px-3 py-1 border border-brand-100 rounded-lg bg-brand-50"
                        >
                          LIQUIDAR
                        </button>
                      )}
